...
dest = "prescaleWeight_reweighted_firstJetPtHLTBin.json"
...
if option.run:
    for aRun, aDataset in runs.items():
        input = "output_rootfile/%s/data/MULTIJET_Data_%s_merged_2012_woPU_pt30_eta50_puJetIdT_firstJetPtHLTBin_beforePrescaleReweighting.root" % (d,aRun)
        #input = "output_rootfile/%s/data_woPrescaleReweighting/MULTIJET_Data_%s_merged_2012.root" % (d,aRun)
        inputs_data.append(input)
else:
    input = "output_rootfile/%s/data/MULTIJET_Data_merged_2012_woPU_pt30_eta50_puJetIdT_firstJetPtHLTBin_beforePrescaleReweighting.root" % (d)
    inputs_data.append(input)

inputs_mc = ["output_rootfile/%s/MC/MULTIJET_MC_QCD-HT_merged_woPU_pt30_eta50_puJetIdT_firstJetPtHLTBin.root" % (d)]
...
#      if Nmc_ref != 0 and Ndata != 0:
          #weight = (Ndata_ref / Nmc_ref) * (Nmc / Ndata)
      #else:
          #weight = 1.
          #if option.verbose:
              #print "WARNING !!! No entry for this bin"
